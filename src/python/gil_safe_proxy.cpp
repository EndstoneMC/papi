...
{
    // Read once, on the registering thread, with the GIL already held by the caller.
    // Caching means later metadata questions never enter Python.
    identifier_ = expansion.getIdentifier();
    author_ = expansion.getAuthor();
    version_ = expansion.getVersion();
    name_ = expansion.getName();
    required_plugin_ = expansion.getRequiredPlugin();
    can_register_ = expansion.canRegister();
    relational_ = expansion.supportsRelationalPlaceholders();
    player_cleanup_ = expansion.supportsPlayerCleanup();
}
